...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    The tidyverse package contains a collection of R packages that helps with cleaning data and analyzing data easily.
...
waste_data_fct <- waste_data |> mutate(
  income_cat = factor(income_cat,levels = levels_income_cat)
)
```
...
#the theme(panel.grid.minor) and theme(panel.grid.major) functions were new to me. Saw them from the complete assignment.
